#!/usr/bin/env python3
"""Sequential manga colorization with fal Qwen Image Edit 2511 and a style reference."""
...
METHOD_NAME = "fal-qwen-image-edit-2511-style-ref-sequential"
DEFAULT_MODEL = "fal-ai/qwen-image-edit-2511"
DEFAULT_STYLE_REF = (
    REPO_ROOT
    / "colorization_methods"
    / "gemini-reference-cache-sequential"
    / "output"
    / "20260808-003106"
    / "0134-008.jpg"
)
...
PRICING = {
    ...
    "date": "2026-08-08",
    "currency": "USD",
    "source": "https://fal.ai/models/fal-ai/qwen-image-edit-2511",
    "usd_per_megapixel": 0.03,
    "input_image_billing": (
        "fal lists $0.03 per megapixel but does not document how input images are "
        "billed; the estimate below assumes each uploaded input image is billed at "
        "its uploaded megapixel count."
    ),
    "notes": (
        "Cost is an estimate only: the endpoint response does not include billed "
        "cost, and failed requests may still incur charges."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Colorize manga pages sequentially with fal Qwen Image Edit 2511, "
            "a labelled character-reference atlas, a style reference page, and "
            "the previous generated page."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Colorized manga page that defines the target colorization style "
            "(palette mood, saturation, shading, screentone treatment). Uploaded "
            "once per run and reused by URL. Pass any colorized page to adjust "
            "the style. Defaults to the Gemini full-run page 8 output."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=4.5,
        help="CFG scale (Qwen model default is 4.5; range 1-20).",
    )
    parser.add_argument(
        "--acceleration",
        choices=("none", "regular", "high"),
        default="regular",
        help="fal acceleration level for this model (default: regular).",
    )
    parser.add_argument(
    ...
    if not 1 <= args.num_inference_steps <= 50:
        parser.error("--num-inference-steps must be between 1 and 50")
    if not 1 <= args.guidance_scale <= 20:
        parser.error("--guidance-scale must be between 1 and 20")
    return args


def style_ref_label(path: Path) -> str:
    """Human-readable label for the style reference image."""
    return f"style_reference:{path.name}"
...
def estimate_cost(
    input_dimensions: list[list[int]], output_dimensions: list[int]
) -> dict[str, Any]:
    input_mp = sum(w * h for w, h in input_dimensions) / 1_000_000
    output_mp = output_dimensions[0] * output_dimensions[1] / 1_000_000
    total_mp = input_mp + output_mp
    estimated_usd = total_mp * PRICING["usd_per_megapixel"]
    return {
        "input_image_count": len(input_dimensions),
        ...
        "note": (
            "fal lists $0.03 per megapixel; the billing basis for input images is "
            "not documented for this model, so this estimate assumes each input "
            "image is billed at its uploaded megapixel count (current page, atlas, "
            "style reference, and previous page are all different sizes)."
        ),
    }
...
def run() -> None:
    ...
    style_ref_path = args.style_ref.resolve()
    if not style_ref_path.is_file():
        raise SystemExit(
            f"Style reference image not found: {style_ref_path}. Pass --style-ref "
            "with a path to a colorized manga page to set the colorization style."
        )
    ...
    manifest: dict[str, Any] = {
        ...
        "preprocessing": {
            ...
            "style_reference": (
                "Colorized page defining the target colorization style, uploaded "
                "once per run and reused by URL. Supplied as the third image on "
                "every request."
            ),
            "source_pages": (
        ...
        "style_reference": {
            "label": style_ref_label(style_ref_path),
            **file_record(style_ref_path),
        },
        "uploads": [],
        "pages": [],
        "totals": {
    ...
    previous_input_dims: list[int] | None = None
    if previous_page_path is not None:
        previous_input_dims = image_mime_and_size(previous_page_path)[1]
    atlas_dims = manifest["reference_atlas"]["dimensions"]
    style_dims = manifest["style_reference"]["dimensions"]
    ...
    try:
        ...
        style_url = fal_client.upload_file(str(style_ref_path))
        manifest["uploads"].append(
            {"role": "style_reference", "path": str(style_ref_path), "url": style_url}
        )
        manifest["totals"]["uploads"] += 1
        if previous_page_path is not None:
        ...
        for index, page_path in enumerate(pages):
            ...
            input_dims = [
                image_mime_and_size(normalized_page)[1],
                atlas_dims,
                style_dims,
            ]
            ...
            cost = estimate_cost(input_dims, output_dimensions)
            ...
            previous_input_dims = output_dimensions
