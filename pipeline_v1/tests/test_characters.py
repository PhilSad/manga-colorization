...
# ---------------------------------------------------------------------------
# page-cast: page-level detection restricted to the per-chapter cast shortlist

def test_page_cast_auto_derives(tmp_path):
    """page-cast without an explicit key auto-derives the cast from the page
    (0134-004 -> c134), renders the page prompt with the shortlist (one text +
    one annotated-page image, no panel-page context), and the record carries
    the effective key."""
    from characters import OpenRouterCharacterDetector
...
def test_page_cast_explicit_key(tmp_path):
    """page-cast with an explicit cast_key honours it over any derivation
    (page would derive c134; the key forces c001)."""
    from characters import OpenRouterCharacterDetector
...
def test_page_cast_no_cast_full_roster(tmp_path):
    """page-cast on a page with no derivable cast falls back to the full
    roster (no shortlist sentence) and keeps working."""
    from characters import OpenRouterCharacterDetector
