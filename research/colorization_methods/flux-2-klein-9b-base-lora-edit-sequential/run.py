...
def build_reference_atlas(refs: list[Path], destination: Path, columns: int) -> None:
    cell_width = 360
    cell_height = 480
    label_height = 36
...
def run() -> None:
    ...
    # Square-ish grid: ceil(sqrt(n)) columns so the labelled atlas reads as a
    # square rather than a single row (4 refs -> 2x2).
    atlas_columns = max(1, math.ceil(math.sqrt(len(refs))))
    build_reference_atlas(refs, atlas_path, atlas_columns)
    ...
    manifest: dict[str, Any] = {
        ...
        "preprocessing": {
            "reference_atlas": f"{atlas_columns}-column labelled JPEG atlas (square grid), uploaded once per run",
            "source_pages": (
