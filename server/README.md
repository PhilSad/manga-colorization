...
## Concurrency and compilation (since 2026-08-14)

The service loads `FLUX2_NUM_PIPES` (default **1** — the original
single-pipe, concurrency-1 server) pipeline instances and hands one out per
in-flight request from a pool; BentoML `traffic concurrency ==
FLUX2_NUM_PIPES`, so concurrent requests never share a pipe (thread safety:
each pipe owns its own mutable scheduler/adapter state, which is why the
per-request `lora_scale` override is safe). Set `FLUX2_NUM_PIPES=2` to enable
the opt-in concurrency-2 mode: two bf16 pipes are ~67 GB resident (within the
GB10's 120 GB unified memory) and `POST /edit2` becomes available — one
request carries two edit jobs (file parts `images1`/`images2`, text fields
`prompt1`/`prompt2`, `seed1`/`seed2`, shared `width`/`height`/
`num_inference_steps`/`guidance_scale`/`lora_scale`/`output_format`), run
concurrently on two pipes, JSON `{"images": [base64, base64],
